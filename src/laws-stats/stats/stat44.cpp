...
// Computation of the value of the test statistic
    ...
    double *Wa;
    Wa = new double[n];
    double statW, tmp=0.0, bhat, muhat=0.0, sumW=0.0, meanW=0.0;
    ...
    R_rsort(Wa,n); // We sort the data, NO NEED SINCE plaplace(x[i]) < plaplace(x[i+1])
	
	// calculate statW
    for (i=1;i<=n;i++) {
	  sumW = sumW + R_pow((Wa[i-1] - (2.0*(double)i-1)/(2.0*(double)n)),2.0);
	  meanW = meanW + Wa[i-1];
	}
	meanW = meanW/double(n);
    
	statW = 1.0/(12.0*(double)n) + sumW - (double)n*R_pow(meanW-0.5,2.0);

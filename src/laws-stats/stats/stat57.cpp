...
// Computation of the value of the test statistic
	...
	// double cos(double x);
	// double sin(double x);
    double statK1, muhat=0.0, bhat, tmp=0.0, tmp2=0.0, sumC=0.0, sumS=0.0, pi;
	...
	bhat = sqrt(tmp2/(2.0*(double)n));
	
	// generate vector Y
    for (i=0;i<n;i++) {
	  Y[i] = plaplace((x[i]-muhat)/bhat);
	}
	
	// calculate statK1
	for (i=0;i<n;i++) {
	  
	  sumC = sumC + cos(2.0*pi*Y[i]);
	  
	  sumS = sumS + sin(2.0*pi*Y[i]);

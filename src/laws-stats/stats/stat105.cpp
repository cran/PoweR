...
extern "C" {
  ...
  void stat105(double *x, int *xlen, double *level, int *nblevel, char **name, int *getname, double *statistic, int *pvalcomp, double *pvalue, double *critvalL, double *critvalR, int *usecrit, int *alter, int *decision, double *paramstat, int *nbparamstat) {
...
// 4: bilateral test that rejects H0 only for small values of the test statistic
    ...
    if (n > 3) {
      ...
      double statV3 = 0.0, statV4 = 0.0, muhat, bhat, tmp = 0.0;
      double *z;
      z = new double[n];
	...
	tmp = tmp + fabs(x[i] - muhat);
      ...
      statistic[0] = R_pow(statV3, 2.0) + R_pow(statV4, 2.0); // Here is the test statistic value	

      if (pvalcomp[0] == 1) {
	// If possible, computation of the p-value.
#include "pvalues/pvalue105.cpp"
      }

...
// Computation of the value of the test statistic
      ...
      // for (i=0;i<=(n-1);i++) m2 = m2 + R_pow(x[i],2.0);
      // m2 = m2/(double)n - R_pow(meanX,2.0); // !! var. pop. ici
      
      // for (i=2;i<=n;i++) {
		// for (j=1;j<=(i-1);j++) {
		  // term1 = term1 + exp((-R_pow(x[j-1]-x[i-1],2.0))/(2.0*m2));
	    // }
      // }
      // term1 = 2.0*term1/(double)n;
	  
      // for (i=0;i<=(n-1);i++) {
		// term2 = term2 + exp((-R_pow(x[i]-meanX,2.0))/(4.0*m2));
      // }
      // term2 = sqrt(2.0)*term2;
	  
	  // statTEP = 1.0 + (double)n/sqrt(3.0) + term1 - term2;
	  ...
	  for (i=0;i<n;i++) {
	    for (j=0;j<n;j++) {
		  term1 = term1 + exp(-0.5*R_pow(x[i]-x[j],2.0)/(R_pow(a,2.0)*S2));
		}
	  }
	  term1 = term1/R_pow((double)n,2.0);
	  
	  for (i=0;i<n;i++) {
	    term2 = term2 + exp(-0.5*R_pow(x[i]-xbar,2.0)/(S2*(1.0+R_pow(a,2.0))));
	  }
	  term2 = 2.0*R_pow(1.0+R_pow(a,-2.0),-0.5)*term2/(double)n;
	  
	  statTEP = term1 - term2 + R_pow(1.0+2.0*R_pow(a,-2.0),-0.5);
	  
	  statTEP = -log((double)n*statTEP);
